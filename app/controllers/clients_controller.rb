class ClientsController < ApplicationController
  ...
  def update
    if @client.update( client_params )
      UserEvent.create( client: @client, description: 'updated your profile', ip_address: request.remote_ip )
  ...
  def change_password
    ...
    if authenticate_user && new_passwords_match
      if @client.update( password: client_params[ :password ] )
        UserEvent.create( client: @client, description: 'changed your password', ip_address: request.remote_ip )
  ...
  def client_params
    params.require( :client ).permit( :username, :password, :old_password, :password_confirmation, :first_name, :last_name, :phone, :email, :sms_consent, :level )
  end
